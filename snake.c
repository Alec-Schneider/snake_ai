...
#define WINDOW_HEIGHT 1000 // 1000
#endif
...
int main(){
    ...
    window = SDL_CreateWindow(
        ...
        WINDOW_WIDTH,
        WINDOW_HEIGHT,
        SDL_WINDOW_BORDERLESS // 0 //  SDL_WINDOW_SHOWN
    );
    // window = SDL_CreateWindow(
    //     "Snake", 
    ...
    // printf("Address of 'window': %p\n", (void*)&window);
    if(!window) {
        fprintf(stderr, "ERROR: !window");
        SDL_Quit(); // Don't forget to clean up and quit SDL
    ...
    if(!renderer){
        ...
        SDL_DestroyWindow(window); // Clean up window before quitting
        SDL_Quit();
        return 1; // Return an error code
    }
    ...
    render_score(renderer, 50, 50);
    render_top_score(renderer, 50, 50 * 3);
